...
| Project | Revision |
| --- | --- |
| MiniExcel-Rust | working tree based on `fcc2a1a` (`0.3.0`) |
...
| Existing-workbook operations | Partial | Rust atomically appends, strictly replaces, or renames worksheets while preserving unrelated package parts and worksheet identity. Copy-and-add, reorder, and standalone visibility mutation remain unsupported. Rename deliberately preserves formula/defined-name text without repairing old-name references, matching .NET `AlterSheet`. |
...
| Multi-sheet and workbook edits | Writer creates multiple sheets; existing workbooks support append, strict replacement, and atomic rename with package preservation | `OpenXmlExporter.InsertSheetAsync`, `CopyAndAddSheetAsync`, `AlterSheetAsync`; `tests/MiniExcel.OpenXml.Tests/MultipleSheets/` and `AlterSheets/` |
...
3. **Remaining workbook edits**: copy/add, reorder, and standalone visibility mutation require their own preservation contracts.
