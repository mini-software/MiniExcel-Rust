...
| Project | Revision |
| --- | --- |
| MiniExcel-Rust | working tree based on `97bcade` (`0.3.0`) |
...
| Existing-workbook operations | Partial | Rust atomically appends, strictly replaces, renames, or changes worksheet visibility while preserving unrelated package parts and worksheet identity. Copy-and-add and reorder remain unsupported. Rename deliberately preserves formula/defined-name text without repairing old-name references, matching .NET `AlterSheet`; visibility rejects hiding the final visible sheet. |
...
| Multi-sheet and workbook edits | Writer creates multiple sheets; existing workbooks support append, strict replacement, atomic rename, and atomic visibility mutation with package preservation | `OpenXmlExporter.InsertSheetAsync`, `CopyAndAddSheetAsync`, `AlterSheetAsync`; `tests/MiniExcel.OpenXml.Tests/MultipleSheets/` and `AlterSheets/` |
...
3. **Remaining workbook edits**: copy/add and reorder require their own preservation contracts.
