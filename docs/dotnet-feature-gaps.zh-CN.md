...
| 项目 | 版本基线 |
| --- | --- |
| MiniExcel-Rust | 基于 `6df7623` 的工作区（`0.3.0`） |
...
| 修改现有工作簿 | 部分实现 | Rust 可原子 append、严格 replace、rename、修改 visibility 或 reorder worksheet，并保留无关 package part 与 worksheet identity。Copy-and-add 仍不支持。Rename 保留 formula 文本；visibility 拒绝隐藏最后一张 visible sheet；reorder 会 remap active/view/local-name index，避免转移其归属。 |
...
| 多工作表与工作簿修改 | Writer 可创建多个工作表；现有 workbook 支持 append、严格 replacement，并以原子 package preservation 覆盖全部独立 `AlterSheet` metadata 操作 | `OpenXmlExporter.InsertSheetAsync`、`CopyAndAddSheetAsync`、`AlterSheetAsync`；`MultipleSheets/` 与 `AlterSheets/` 测试 |
...
3. **剩余 workbook edit**：copy-and-add 需要 relationship closure cloning contract。
