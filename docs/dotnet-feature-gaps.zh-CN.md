...
| 项目 | 版本基线 |
| --- | --- |
| MiniExcel-Rust | 基于 `6a7ea00` 的工作区（`0.3.0`） |
...
| 模板 | 部分实现 | Rust 可使用 scalar、array 与 enumerable-cell conditional block 填充 path/byte template 并保留 package part；path output 也提供 cancellable async wrapper。Stream、grouping、nested/logical condition、参数化 sheet、`$=` formula、formula reference 调整和 calculation chain 处理仍不支持。 |
...
2. **高级模板与 Fluent Mapping**：通过独立兼容里程碑增加 grouped template、更丰富 conditional expression、参数化 sheet 和 mapping。
