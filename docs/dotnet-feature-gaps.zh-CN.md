...
| 读取配置 | 部分实现 | 区域文化感知转换、共享字符串缓存控制、缓冲/快速模式，以及部分 null/空字符串行为。合并单元格值填充已实现。 |
| 写入配置与样式 | 部分实现 | 表格/自动筛选、从右到左工作表、冻结行列、自动列宽、表头样式/对齐/换行、共享字符串与内联字符串选择，以及更广泛的单元格样式。Rust 当前主要暴露工作表名、是否输出表头和数字格式。 |
