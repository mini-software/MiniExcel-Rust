...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| Cold | .NET v1 | 1,714.86 ms | 58,314 行/秒 | 1,769.77 ms | 66.84 MB | 68.19 MB |
| Cold | Rust | 674.59 ms | 148,238 行/秒 | 681.71 ms | 9.64 MB | 9.72 MB |
| Steady | .NET v1 | 2,330.46 ms | 128,730 行/秒 | 4,069.78 ms | 80.35 MB | 84.28 MB |
| Steady | Rust | 2,079.01 ms | 144,299 行/秒 | 2,792.32 ms | 9.88 MB | 9.93 MB |

对于新进程中的首次 Query，Rust 吞吐量为 .NET v1 的 2.54 倍，Query 耗时少 60.7%，峰值工作集少 85.6%。

进程内 warm-up 后，Rust 吞吐量为 .NET v1 的 1.12 倍，Query 耗时少 10.8%；峰值工作集中位数低 87.7%，为 9.88 MB 对 80.35 MB。

与 Rust 优化基线相比，Cold 场景的 Query 耗时中位数下降 53.7%，Steady 场景下降 50.6%，且峰值工作集没有增加。
