...
| --- | --- | ---: | ---: | ---: | ---: | ---: |
| Cold | .NET v1 | 1,731.77 ms | 57,744 行/秒 | 1,784.10 ms | 67.75 MB | 68.13 MB |
| Cold | Rust | 670.45 ms | 149,154 行/秒 | 675.49 ms | 6.86 MB | 7.08 MB |
| Steady | .NET v1 | 2,230.25 ms | 134,514 行/秒 | 4,044.79 ms | 80.62 MB | 83.86 MB |
| Steady | Rust | 2,081.60 ms | 144,120 行/秒 | 2,815.70 ms | 6.71 MB | 7.20 MB |

对于新进程中的首次 Query，Rust 吞吐量为 .NET v1 的 2.58 倍，Query 耗时少 61.3%，峰值工作集少 89.9%。

进程内 warm-up 后，Rust 吞吐量为 .NET v1 的 1.07 倍，Query 耗时少 6.7%；峰值工作集中位数低 91.7%，为 6.71 MB 对 80.62 MB。

与 Rust 优化基线相比，Cold 场景的 Query 耗时中位数下降 54.0%，Steady 场景下降 50.5%，峰值工作集保持在 8 MB 以下。
...
内存型 shared strings 现在使用一个连续 UTF-8 缓冲区和结束偏移表，不再为每个唯一字符串分别进行堆分配。偏移表根据受文件大小约束的 `uniqueCount` 预分配；不含 Excel `_xHHHH_` 转义的字符串会直接复用原有分配。当合并后的字符串数据不超过 4 GiB 时，偏移使用 `u32`；超过该边界则自动扩展为 `usize`，因此不会限制超大输入。

与上一轮优化结果相比，自适应偏移表示让 Cold 和 Steady 的峰值工作集中位数分别再下降 5.1% 和 8.8%。最后两轮同时间段交替 A/B 中，Query 耗时与 `usize` 基线的差异约在 1% 内，且没有一致回退；上表跨轮次的耗时差异仍会受到机器负载波动影响。
