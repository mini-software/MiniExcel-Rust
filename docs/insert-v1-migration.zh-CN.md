...
Insert 创建、append 或严格 replace 完整 worksheet。它不会向现有 worksheet 追加 row，不编辑 macro，不计算公式，不复制任意 sheet，不重命名/重排 sheet，也不是通用 workbook editor。`.xlsm` package 与 Strict OOXML package 会被拒绝。
